...
class BakeCommand(Command):
    ...
    def run(self, args: argparse.Namespace) -> int:
        ...
        if not recipe:
            logger.error("bake needs 'build.recipe' (a Dockerfile path) in config")
            return 1
        if not image:
            logger.error("bake needs an 'image' tag in config")
            return 1
        runtime = config.get("runtime", "auto")
        if runtime != "auto" and validate(image, runtime) == "incompatible":
            logger.error("runtime %r cannot use a built OCI image", runtime)
            return 1
        builder = build.get("builder") or detect_builder()
        if not builder:
            logger.error(
                "no OCI image builder found on PATH (looked for: %s)",
                ", ".join(OCI_BUILDERS),
            )
            return 1
